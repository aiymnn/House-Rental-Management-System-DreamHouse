            ...
            <div class="d-flex justify-content-start">
                ...
                <div class="container mt-3">
                    <div class="agent-details-row">
                        ...
                        <div class="agent-details-value">{{ Auth::guard('tenant')->user()->name }}</div>
                    </div>
                    <div class="agent-details-row">
                        <div class="agent-details-label">Email</div>
                        <div class="agent-details-value">{{ Auth::guard('tenant')->user()->email }}</div>
                    </div>
                    <div class="agent-details-row">
                        <div class="agent-details-label">I/C Number</div>
                        <div class="agent-details-value">{{ substr(Auth::guard('tenant')->user()->number_ic, 0, 6) }}-{{ substr(Auth::guard('tenant')->user()->number_ic, 6, 2) }}-{{ substr(Auth::guard('tenant')->user()->number_ic, 8) }}</div>
                    </div>
                    <div class="agent-details-row">
                        <div class="agent-details-label">Phone</div>
                        <div class="agent-details-value">{{ substr(Auth::guard('tenant')->user()->phone, 0, 3) }}-{{ substr(Auth::guard('tenant')->user()->phone, 3) }}</div>
                    </div>
                </div>
            </div>
        </div>
                ...
                <tbody>
                    @foreach($contracts as $contract)
                        <tr>
                            ...
                            <td>{{ $contract->property->monthly }}</td>
