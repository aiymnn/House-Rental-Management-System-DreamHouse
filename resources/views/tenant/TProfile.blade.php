...
<div class="details-container">
    ...
    <div class="card-body" id="paymentDetailBody">
        <div class="container text-center mb-3">
            <form id="avatar-form" action="{{ url('tenant/profile/avatar/'.Auth::guard('tenant')->user()->id.'') }}" enctype="multipart/form-data" method="POST">
                @csrf
                <h1 class="mb-3">{{ Auth::guard('tenant')->user()->name }}</h1>
                @if(Auth::guard('tenant')->check())
                <div class="avatar-container position-relative">
                    <label for="avatar-upload" class="edit-icon"><i class="fas fa-pencil-alt"></i></label>
                    @if(Auth::guard('tenant')->user()->avatar)
                        <img src="{{ asset(Auth::guard('tenant')->user()->avatar) }}" alt="Avatar" class="rounded-circle circle-image mb-3" id="avatar-image">
                    @else
          ...
          <div class="container mb-3">
            ...
            <h4>Your Profile</h4>
            <hr>
            <form action="{{ url('tenant/profile/update/'.Auth::guard('tenant')->user()->id.'')}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label class="form-label">Name</label>
                    <input type="text" class="form-control" aria-describedby="emailHelp" name="name" value="{{ Auth::guard('tenant')->user()->name }}">
                    @error('name') <span class="text-danger">{{ $message }}</span>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Phone</label>
                    <input type="number" class="form-control" aria-describedby="emailHelp" name="phone" value="{{ Auth::guard('tenant')->user()->phone }}">
                    @error('phone') <span class="text-danger">{{ $message }}</span>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">IC Number</label>
                    <input type="email" class="form-control" aria-describedby="emailHelp" name="icnumber" value="{{ Auth::guard('tenant')->user()->number_ic }}" readonly>
                    @error('icnumber') <span class="text-danger">{{ $message }}</span>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">I/C Number</label>
                    <input type="email" class="form-control" aria-describedby="emailHelp" name="icnumber" value="{{ substr(Auth::guard('tenant')->user()->number_ic, 0, 6) }}-{{ substr(Auth::guard('tenant')->user()->number_ic, 6, 2) }}-{{ substr(Auth::guard('tenant')->user()->number_ic, 8) }}" readonly>
                    @error('icnumber') <span class="text-danger">{{ $message }}</span>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Email</label>
                    <input type="email" class="form-control" aria-describedby="emailHelp" name="email" value="{{ Auth::guard('tenant')->user()->email }}" readonly>
          ...
          <div class="container mb-3">
            ...
            <hr>
            <form action="{{ url('tenant/profile/password/'.Auth::guard('tenant')->user()->id.'')}}" method="POST" enctype="multipart/form-data">  <!-- Ensure the action is correctly set -->
                @csrf
                <div class="mb-3">
