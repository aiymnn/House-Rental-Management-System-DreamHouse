    ...
    <div class="card-body" id="paymentDetailBody">
        <div class="container text-center mb-3">
          ...
          <div class="container mb-3">
            ...
            <h4>Your Profile</h4>
            <hr>
            <form action="{{ url('agent/profile/update/'.Auth::guard('staff')->user()->id.'')}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="email" class="form-control" aria-describedby="emailHelp" name="icnumber" value="{{ substr(Auth::guard('staff')->user()->number_ic, 0, 6) }}-{{ substr(Auth::guard('staff')->user()->number_ic, 6, 2) }}-{{ substr(Auth::guard('staff')->user()->number_ic, 8) }}" readonly>
                    @error('icnumber') <span class="text-danger">{{ $message }}</span>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Email</label>
                    <input type="email" class="form-control" aria-describedby="emailHelp" name="email" value="{{ Auth::guard('staff')->user()->email }}" readonly>
          ...
          <div class="container mb-3">
            ...
            <hr>
            <form action="{{ url('agent/profile/password/'.Auth::guard('staff')->user()->id.'')}}" method="POST" enctype="multipart/form-data">  <!-- Ensure the action is correctly set -->
                @csrf
                <div class="mb-3">
