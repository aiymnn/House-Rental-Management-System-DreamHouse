...
<div class="container contract-detail-container">
    ...
    <div class="contract-detail-row">
        <div class="contract-detail-label">Address<span style="color: #023d90"><a target="__blank" href="{{ url('staff/property/view/'.$property->id.'') }}" style="margin-left: 5px;" title="view property"><i class="fa fa-search" aria-hidden="true"></i></a></span></div>
    ...
    <div class="card-body" style="display: none;" id="agentDetailBody">
        <div class="d-flex justify-content-start">
            <div class="avatar-container">
                <img src="{{ asset($contract->agent->avatar) }}" alt="Avatar">
            </div>
            <div class="container mt-3">
                <div class="agent-details-row">
                    <div class="agent-details-label">Name</div>
                    <div class="agent-details-value">{{ $contract->agent->name }}</div>
                </div>
                <div class="agent-details-row">
                    <div class="agent-details-label">I/C Number</div>
                    <div class="agent-details-value">{{ substr($contract->agent->number_ic, 0, 6) }}-{{ substr($contract->agent->number_ic, 6, 2) }}-{{ substr($contract->agent->number_ic, 8) }}</div>
                </div>
                <div class="agent-details-row">
                    <div class="agent-details-label">Email</div>
                    <div class="agent-details-value">{{ $contract->agent->email }}</div>
                </div>
                <div class="agent-details-row">
                    <div class="agent-details-label">Phone</div>
                    <div class="agent-details-value">{{ substr($contract->agent->phone, 0, 3) }}-{{ substr($contract->agent->phone, 3) }}</div>
                </div>
                <div class="agent-contact-buttons">
                    @php
                            // Assuming $phoneNumber contains the phone number fetched from the database
                            $phoneNumber = $contract->agent->phone; // Example phone number
                    ...
                    <a href="https://mail.google.com/mail/?view=cm&fs=1&to={{ $contract->agent->email }}" class="btn agent-contact-button email-button" target="_blank">
...
<div class="container agent-details-container">
    <div class="agent-details-header" id="tenantDetailHeader">
        <h4>Tenant Detail</h4>
    </div>
    <div class="card-body" style="display: none;" id="tenantDetailBody">
        <div class="d-flex justify-content-start">
            <div class="avatar-container">
                @if($contract->tenant->avatar)
                    <img src="{{ asset($contract->tenant->avatar) }}" alt="Avatar">
                @else
            ...
            <div class="container mt-3">
                <div class="agent-details-row">
                    <div class="agent-details-label">Name</div>
                    <div class="agent-details-value">{{ $contract->tenant->name }}</div>
                </div>
                <div class="agent-details-row">
                    <div class="agent-details-label">I/C Number</div>
                    <div class="agent-details-value">{{ substr($contract->tenant->number_ic, 0, 6) }}-{{ substr($contract->tenant->number_ic, 6, 2) }}-{{ substr($contract->tenant->number_ic, 8) }}</div>
                </div>
                <div class="agent-details-row">
                    <div class="agent-details-label">Email</div>
                    <div class="agent-details-value">{{ $contract->tenant->email }}</div>
                </div>
                <div class="agent-details-row">
                    <div class="agent-details-label">Phone</div>
                    <div class="agent-details-value">{{ substr($contract->tenant->phone, 0, 3) }}-{{ substr($contract->tenant->phone, 3) }}</div>
                </div>
                <div class="agent-contact-buttons">
                    @php
                            // Assuming $phoneNumber contains the phone number fetched from the database
                            $phoneNumber = $contract->tenant->phone; // Example phone number
                    ...
                    <a href="https://mail.google.com/mail/?view=cm&fs=1&to={{ $contract->tenant->email }}" class="btn agent-contact-button email-button" target="_blank">
                        <i class="fa-solid fa-envelope"></i> <!-- Email Icon -->
                    </a>
                </div>
            </div>
...
<script>
    $(document).ready(function(){
        $("#tenantDetailHeader").click(function(){
            $("#tenantDetailBody").slideToggle();
        });
    });
</script>
