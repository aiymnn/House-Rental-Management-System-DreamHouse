...
@section('title', 'DreamHouse • Agent')
    ...
    <div class="card-body" id="paymentDetailBody">
        @if($agents->isEmpty())
            <div class="text-center p-3">
                <h3 class="text-secondary">You don't have any agents yet.</h3>
            </div>
        @else
        <table class="details-table">
            ...
            <tbody id="agentsTableBody">
                ...
                <tr>
                    ...
                    <td style="text-align: center">{{ substr($agent->number_ic, 0, 6) }}-{{ substr($agent->number_ic, 6, 2) }}-{{ substr($agent->number_ic, 8) }}</td>
                    <td style="text-align: center">{{ substr($agent->phone, 0, 3) }}-{{ substr($agent->phone, 3) }}</td>
                    <td style="text-align: center">{{$agent->email}}</td>
                    <td style="text-align: center">{{ $agent->assigned_properties_count }}</td>
                    <td style="text-align: center">{{ $agent->handled_contracts_count }}</td>
                    <td style="text-align: center">
                        @if ($agent->status == 1)
                        <a href="{{ url('staff/agent/offline/'.$agent->id.'')}}" class="btn btn-success">Online</a>
                        {{-- <a href="{{ url('staff/property/approval/'.$property->id.'') }}" class="btn btn-success">Approval</a> --}}
                        @else
                        <a href="{{ url('staff/agent/online/'.$agent->id.'')}}" class="btn btn-danger">Offline</a>
                        @endif
                    </td>
                    <td style="text-align: center">{{ \Carbon\Carbon::parse($agent->created_at)->format('d-m-Y') }}</td>
                    <td style="text-align: center">
                        <a href="{{ url('staff/user/agent/view/'.$agent->id.'') }}" class="col" style="color:green; margin-right: 10px;" title="View"><span><i class="fa-solid fa-eye"></i></span></i></a>
                        <a href="{{ url('staff/user/agent/edit/'.$agent->id.'') }}" class="col" style="color:blue; margin-right: 10px;" title="Edit"><span><i class="fa-solid fa-pen-to-square"></i></span></a>
        ...
        <p id="notFoundMessage" class="text-center d-none">Not found</p>
        @endif
    </div>
</div>
<!-- JavaScript for filtering -->
<script>
    // Function to filter agents based on search input and checkboxes
    function filterAgents() {
        ...
        Array.from(agentsTableBody.children).forEach(row => {
            const name = row.children[1].innerText.toLowerCase();
            const icNumber = row.children[2].innerText.toLowerCase();
            const status = row.children[7].innerText.toLowerCase();
        ...
        // Show "Not found" message if no agents are found
        notFoundMessage.classList.toggle('d-none', foundAgents > 0);
    }

    // Add event listeners to trigger filtering
    document.getElementById('searchInput').addEventListener('input', filterAgents);
    document.getElementById('onlineCheckbox').addEventListener('change', filterAgents);
    document.getElementById('offlineCheckbox').addEventListener('change', filterAgents);
