...
<div class="details-container">
    ...
    <div class="details-header" id="paymentDetailHeader">
        <h4>Tenant Profile Configuration
        </h4>
    </div>
    <div class="card-body" id="paymentDetailBody">
        <div class="container text-center mb-3">
            <form id="avatar-form" action="{{ url('staff/profile/tenant/avatar/'.$tenant->id.'') }}" enctype="multipart/form-data" method="POST">
                @csrf
                <h1 class="mb-3">{{ $tenant->name }}</h1>
                <div class="avatar-container position-relative">
                    <label for="avatar-upload" class="edit-icon"><i class="fas fa-pencil-alt"></i></label>
                    @if($tenant->avatar)
                        <img src="{{ asset($tenant->avatar) }}" alt="Avatar" class="rounded-circle circle-image mb-3" id="avatar-image">
                    @else
          ...
          <div class="container mb-3">
            ...
            <h4>Your Profile</h4>
            <hr>
            <form action="{{ url('staff/profile/tenant/bio/'.$tenant->id.'') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label class="form-label">Name</label>
                    <input type="text" class="form-control" aria-describedby="emailHelp" name="name" value="{{ $tenant->name }}">
                    @error('name') <span class="text-danger">{{ $message }}</span>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Phone</label>
                    <input type="number" class="form-control" aria-describedby="emailHelp" name="phone" value="{{ $tenant->phone }}">
                    @error('phone') <span class="text-danger">{{ $message }}</span>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">I/C Number</label>
                    <input type="number" class="form-control" aria-describedby="emailHelp" name="icnumber" value="{{ $tenant->number_ic }}">
                    @error('icnumber') <span class="text-danger">{{ $message }}</span>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Email</label>
                    <input type="email" class="form-control" aria-describedby="emailHelp" name="email" value="{{ $tenant->email }}">
          ...
          <div class="container mb-3">
            ...
            <hr>
            <form action="{{ url('staff/profile/tenant/password/'.$tenant->id.'') }}" method="POST" enctype="multipart/form-data">  <!-- Ensure the action is correctly set -->
                @csrf
                <div class="mb-3">
